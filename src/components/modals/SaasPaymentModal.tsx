  ...
  const handleBrickSuccess = async (token: string | undefined, issuer_id?: string, payment_method_id?: string, card_holder_name?: string, identification?: any) => {
    ...
    try {
        ...
        if (status === 'approved') {
            ...
            const msg = data.status_detail 
                ? `Pagamento rejeitado: ${getPaymentStatusDetail(data.status_detail)}` 
                : 'Pagamento rejeitado pela operadora.';
            toast.error(msg, { duration: 5000 });
            throw new Error(msg); // Throw to Brick to keep modal open
        } else {
             ...
             if (status === 'in_process') {
                 ...
                 // Should we close? Maybe not if unknown. But usually safe to close if not rejected.
                 await onSuccess(); 
                 onClose();
             }
        }
        ...
        throw error; // Propagate to Brick
    } finally {
        setLoading(false);
    }
  };

  const getPaymentStatusDetail = (detail: string) => {
    const map: any = {
        ...
        'cc_rejected_card_disabled': 'Cartão desativado. Ligue para sua operadora.',
    ...
    return map[detail] || 'Verifique os dados ou tente outro cartão.';
  };
